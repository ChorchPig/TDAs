...
int sumaElementosVector(Vector *vector){
    int resultado=0, n=getCurrentSize(vector);
    for(int i=0; i<n; i++)resultado+=vector->arreglo[i];
    return resultado;
}
...
int apareceEnVector(Vector *vector, VECTOR_ELEMENT valor){
    int apariciones=0, n=getCurrentSize(vector);
    for(int i=0; i<n; i++){
        if(vector->arreglo[i]==valor)apariciones++;
    }
    return apariciones;
}
...
void agregarYOrdenarVector(Vector *vector, VECTOR_ELEMENT valor){
    agregarAlVector(vector, valor);
    ordenarVector(vector);
}

Vector* generarVectorOrdenado(Vector *vector){
    Vector *copiaVector=copiarVector(vector);
    ordenarVector(copiaVector);
    return copiaVector;
}

Vector* copiarVector(Vector *vector){
    Vector *copiaVector=crearVector(getCurrentSize(vector));
    int n=getMaxSize(copiaVector);
    for(int i=0; i<n; i++){
        agregarAlVector(copiaVector, vector->arreglo[i]);
    }
    return copiaVector;
}
